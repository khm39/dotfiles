...
description: >
  ...
  - 「Codexにレビューさせて」「Codexの意見を聞いて」「セカンドオピニオンが欲しい」
  ...
  ユーザーが「MCP」と明示しなくても、Codex への委譲・相談・レビュー依頼であればこのスキルを使う。
---
...
Codex は ChatGPT もしくは API キーでのログインが必要。疑わしいときは確認する：

```bash
codex login status
```

未ログインなら `codex login` をユーザーに案内する（対話ログインなので、プロンプトで `! codex login` を実行してもらう）。
...
補足：差分に対する**定型のレビュー出力**（`[P1]`/`[P2]` 等の重大度タグ付き）が欲しい場合は、
MCP ではなく CLI の専用レビューアも使える：`codex review --uncommitted` / `--base <branch>` /
`--commit <sha>`。MCP 経由で十分なことが多いが、構造化された出力が要るときの選択肢として覚えておく。

## 出力の扱いと失敗検知

- `mcp__codex__codex` の呼び出しは **Codex がタスクを終えるまでブロックする**。大きめの実装は数分かかることがある。
- 戻り値の最終メッセージが**空でないこと**、Codex が「できなかった」と報告していないかを確認してから「完了」と判断する。サンドボックスにブロックされた実行は、見かけ上は成功に見えることがある。
