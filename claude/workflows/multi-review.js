...
const DIMENSIONS = [
  ...
  { key: 'maintainability', label: '保守性・設計', focus: '可読性、命名、重複コード、過度な複雑さ、責務の混在、不要なコメント、抽象化の過不足、既存パターンからの逸脱。' },
...
function reportPrompt(confirmed, scope) {
  return [
    ...
    '',
    'レポート構成:',
    '1. 冒頭に総評(2〜4行)と総合判定: ✅ Approve / 🔧 Request changes / ⛔ Block のいずれか。critical/highが残る場合は Approve にしない。',
    '2. 深刻度の高い順(critical→high→medium→low→nit)に指摘を列挙。各指摘は見出しに [深刻度] file:line とタイトル、本文に理由と具体的な修正案を書き、観点(dimension)も併記する。',
    '3. 全体として良い点があれば簡潔に触れる。',
    '4. 指摘が1件も無ければ、変更範囲を確認した上で「問題なし」と明記する。',
    '',
